...
export default function TopNav({ open, onClose }) {
  ...
  const navItems = [
    ...
    { to: "/testing-requests", icon: ShieldCheck, label: "Testing Request" },
  ...
  const visibleItems = isAdmin
    ? navItems
    : navItems.filter(({ label }) => baseAllowed.has(label) || (Array.isArray(allowedModules) && allowedModules.includes(label)));
      ...
      <div>
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-white/10 bg-gradient-to-r from-primary to-primaryLight sticky top-0 z-20">
          <Link to="/" className="flex items-center gap-2">
            ...
            {role ? (
              <span className="ml-2 inline-flex items-center rounded-md bg-white/10 px-2 py-1 text-xs font-semibold text-white">
                {role}
              </span>
            ) : null}
          </Link>
